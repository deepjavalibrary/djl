...
Using DJL, you can create, train, and run inference with deep learning models.

In this tutorial, you will create your first application using DJL for deep learning inference.
Your application will implement an [Object Detection application](https://en.wikipedia.org/wiki/Object_detection) based on a pre-trained 
ResNet-50 SSD model.

## Prerequisites
Before you start, review the JavaDocs for the following classes.
...
pre-defined model. DJL abstracts away the whole process for ease of use. DJL can load the model, perform
inference on the input, and provide output. DJL also allows user-defined inputs. 

The red block ("Images") in the workflow is the input that DJL expects from you. The green block
("Images bounding box") is the output DJL . Since DJL does not know which input to
expect and which format of output that you prefer, DJL provides the `Translator` interface so you
...
to run inference on an image. This tutorial is based on the MxNet ResNet-50 SSD model for object detection. The code
...
[ImageNet database](http://www.image-net.org) and has 50 layers. It can detect objects
in images and classify the objects into different object categories. This tutorial uses a
ResNet-50 SSD model that has already been trained on MXNet to extract features from the input
and perform its function. This saves time and effort for you. Deep Learning frameworks like MXNet,
PyTorch, and TensorFlow all offer pre-trained models. Each of the frameworks
...
the MXNet ResNet-50 SSD pre-trained model, run the following commands. Note the directory
that the models are downloaded into. You will need to provide the path to the directory while
loading the model.
 ...
 The symbol and params files together represent the trained neural network. The `synset.txt` file is the mapping
...
converts user-defined input and output objects to NDList and vice versa. Translator
interface has two methods that need to be implemented: `processInput()` for pre-processing and `processOutput()` for
post-processing. This is represented by the two white boxes in the image. These are the two main blocks of code that
...
[Batch learning](https://medium.com/@divakar_239/stochastic-vs-batch-gradient-descent-8820568eada1) helps take
...
The input of the translator should be a buffered image or any type that can load an image. For the sake of 
simplicity, we are using a Batch size of 1. Channel is usually 3(RGB). For Height and Width, we recommend using
(512, 512) since the model was trained on similar images. You can also resize the image during pre-processing to fit the
required dimensions.
...
`modelDir` is the path to the directory where the model was downloaded. The `modelName` can generally be inferred from the 
name of the symbol or param files that were downloaded. In this case, it is `resnet50_ssd_model`. Later, 
we will pass these values using command-line arguments.  
~~~
...
You can use the Predictor created previously to run inference in one single step!
~~~
List<DetectedObject> predictResult = predictor.predict(img);
~~~

### SSD Sample Implementation
All previous instructions to build inference can be found in the [Sample SSD implementation](../src/main/java/ai/djl/examples/inference/SsdExample.java).
The sample SSD code provided in this module applies the bounding boxes to a copy of the original image and stores the result in
...
You can navigate to the source folder, and use the following command to run inference:
 
 ```
 ./gradlew -Dmain=ai.djl.examples.inference.ObjectDetection run --args="-i {PATH_TO_IMAGE} -l {OUTPUT_DIR}" 
 ```
 
When you run inference on the image of the dog and the cat, your output should look like the following:
