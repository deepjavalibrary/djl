...
```
curl -O https://publish.djl.ai/djl-bench/0.14.1/djl-bench_0.14.1-1_all.deb
sudo dpkg -i djl-bench_0.14.1-1_all.deb
```

For centOS or Amazon Linux 2

You can download djl-bench zip file from [here](https://publish.djl.ai/djl-bench/0.14.1/benchmark-0.14.1.zip).

```
curl -O https://publish.djl.ai/djl-bench/0.14.1/benchmark-0.14.1.zip
unzip benchmark-0.14.1.zip
rm benchmark-0.14.1.zip
sudo ln -s $PWD/benchmark-0.14.1/bin/benchmark /usr/bin/djl-bench
```

For Windows

We are considering to create a `chocolatey` package for Windows. For the time being, you can
download djl-bench zip file from [here](https://publish.djl.ai/djl-bench/0.14.1/benchmark-0.14.1.zip).
