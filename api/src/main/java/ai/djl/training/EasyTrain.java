...
public final class EasyTrain {
    ...
    public static final EasyTrainer TRAINER = new EasyTrainer();
    ...
    public static void fit(
        ...
        TRAINER.fit(trainer, numEpoch, trainingDataset, validateDataset);
    }
    ...
    public static void trainBatch(Trainer trainer, Batch batch) {
        TRAINER.trainBatch(trainer, batch);
    }
    ...
    public static void validateBatch(Trainer trainer, Batch batch) {
        TRAINER.validateBatch(trainer, batch);
    }
    ...
    public static void evaluateDataset(Trainer trainer, Dataset testDataset)
            throws IOException, TranslateException {
        TRAINER.evaluateDataset(trainer, testDataset);
    }

    /** Helper for easy training of a whole model, a trainining batch, or a validation batch. */
    public static class EasyTrainer {
        ...
        public void fit(
            ...
            // Deep learning is typically trained in epochs where each epoch trains the model on
            // each
         ...
         * Validates the given batch of data.
         *
         * <p>During validation, the evaluators and losses are computed, but gradients aren't
         ...
         * @throws IllegalArgumentException if the batch engine does not match the trainer engine
         */
        public void validateBatch(Trainer trainer, Batch batch) {
        ...
        protected boolean validateSplit(Trainer trainer, BatchData batchData, Batch split) {
            NDList data = split.getData();
            NDList labels = split.getLabels();
         ...
         * @throws TranslateException if there is an error while processing input
         */
        public void evaluateDataset(Trainer trainer, Dataset testDataset)
                throws IOException, TranslateException {
