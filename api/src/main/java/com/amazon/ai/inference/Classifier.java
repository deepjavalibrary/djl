...
public class Classifier<I, O> implements AutoCloseable {
    ...
    public Classifier(Model model, Translator<I, O> transformer) {
        this(model, transformer, Context.defaultContext());
    }

    public Classifier(Model model, Translator<I, O> transformer, Context context) {
        this.predictor = Predictor.newInstance(model, transformer, context);
    }

    public O classify(I input) throws TranslateException {
