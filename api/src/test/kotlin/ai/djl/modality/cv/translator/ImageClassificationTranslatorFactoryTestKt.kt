...
class ImageClassificationTranslatorFactoryTestKt {
    private lateinit var factory: ImageClassificationTranslatorFactory

    @BeforeClass
    fun setUp() {
        factory = ImageClassificationTranslatorFactory()
    }
    ...
    fun testNewInstance() {
        ...
        Model.newInstance("test").use { model ->
            val translator1 = factory<Image, Classifications>(model, arguments)
            Assert.assertTrue(translator1 is ImageClassificationTranslator)

            val translator2 = factory<Path, Classifications>(model, arguments)
            Assert.assertTrue(translator2 is BasicTranslator<*, *>)

            val translator3 = factory<URL, Classifications>(model, arguments)
            Assert.assertTrue(translator3 is BasicTranslator<*, *>)

            val translator4 = factory<InputStream, Classifications>(model, arguments)
