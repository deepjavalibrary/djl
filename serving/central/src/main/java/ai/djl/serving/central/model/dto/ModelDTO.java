 ...
 * 
 * this class inherits from ModelReferenceDTO and can so be used as a ModelReference.
 * 
...
public class ModelDTO extends ModelReferenceDTO {
	...
	public ModelDTO(Artifact artifact) {
		...
		licenses=Collections.unmodifiableMap(artifact.getMetadata().getLicenses().values().stream().collect(Collectors.toMap(License::getName, License::getUrl)));
		...
		if (artifact.getFiles()!=null) {
			files=artifact.getFiles().entrySet().stream().map( e-> new FileResourceDTO(e.getKey(), e.getValue())).collect(Collectors.toList());
		} else {
