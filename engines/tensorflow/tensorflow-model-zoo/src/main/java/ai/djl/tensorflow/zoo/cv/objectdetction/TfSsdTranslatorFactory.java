...
public class TfSsdTranslatorFactory extends ObjectDetectionTranslatorFactory {
    ...
    public Translator<?, ?> newInstance(
        ...
        if (input == Image.class && output == DetectedObjects.class) {
            ...
            return new FileTranslator<>(TfSsdTranslator.builder(arguments).build());
        } else if (input == URL.class && output == DetectedObjects.class) {
            return new UrlTranslator<>(TfSsdTranslator.builder(arguments).build());
        } else if (input == InputStream.class && output == DetectedObjects.class) {
            return new InputStreamTranslator<>(TfSsdTranslator.builder(arguments).build());
        } else if (input == Input.class && output == Output.class) {
            return new ImageServingTranslator(TfSsdTranslator.builder(arguments).build());
        }
