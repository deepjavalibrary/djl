...
JNIEXPORT jlong JNICALL Java_ai_djl_pytorch_jni_PyTorchLibrary_torchStft(
        JNIEnv* env, jobject jthis, jlong jhandle, jlong jn_fft, jlong jhop_length, jlong jwindow, jboolean jcenter, jboolean jnormalize, jboolean jreturn_complex) {
    API_BEGIN()
        const auto* tensor_ptr = reinterpret_cast<torch::Tensor*>(jhandle);
        if (jwindow == -1L) {
            const auto* result_ptr = new torch::Tensor(tensor_ptr->stft(jn_fft, jhop_length, c10::nullopt, c10::nullopt, jcenter, "reflect", jnormalize, c10::nullopt, jreturn_complex));
            return reinterpret_cast<uintptr_t>(result_ptr);
        } else {
            const auto* window_ptr = reinterpret_cast<torch::Tensor*>(jwindow);
            const auto* result_ptr = new torch::Tensor(tensor_ptr->stft(jn_fft, jhop_length, c10::nullopt, *window_ptr, jcenter, "reflect", jnormalize, c10::nullopt, jreturn_complex));
            return reinterpret_cast<uintptr_t>(result_ptr);
        }
    API_END_RETURN()
}
